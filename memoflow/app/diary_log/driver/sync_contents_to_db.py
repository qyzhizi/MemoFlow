...
def texts_to_matching_lines(content: str):
    # split contents to lines, splitlines() 方法用于按照行('\r', '\r\n', \n')分隔,
    # 返回一个包含各行作为元素的列表
...
def get_new_card_content_no_uuid(all_card_content):
    new_all_card_content = []
    for content in all_card_content:
        # Generate UUID for the new record
        tags = get_tags_from_content(content)
        new_tags = ','.join(tags)
        new_all_card_content.append((content, new_tags))
    return new_all_card_content
...
def text_to_card_contents(content: str):
    ...
    if not content.startswith('- '):
        content = '- ' + content
    matches, all_lines = texts_to_matching_lines(content)
    ...
    all_card_content.reverse()
    # 生成新的内容，添加了tags
    new_all_card_content = get_new_card_content_no_uuid(
        all_card_content=all_card_content)
