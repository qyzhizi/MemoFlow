...
- 3.2、坚果云文件同步
    ...
    **(1) 获取坚果云api token**
    ...
    坚果云提供了 RESTful 风格的 API，可以通过 API 访问坚果云的文件和文件夹。要使用坚果云 API，需要进行以下步骤：
    ...
    - 点击「创建应用」按钮，填写应用名称、应用描述等信息，然后点击「确定」。

    - 在应用页面中，找到「API 接入」部分，可以看到应用的 App Key 和 App Secret。

    - 在应用页面中，点击「生成 Access Token」按钮，生成 Access Token。
